...
data is read from I2C bus. After successful reading , packets are formed in order to send it through the UART bus to an external device. In this case , the external device used is nrf DK board. In nrf DK application, the received information is first evaluated by performing certain checks and then the data is unpacked and displayed on the logger window.
